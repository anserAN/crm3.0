...
public class UsersServiceImpl implements UsersService {
    ...
    public Map<String, Object> phoneChange(Map<String, Object> query) {
        ...
        //标志
        int flag = 0;
        /*if (query.get("phone")!=null && query.get("phone").toString()!="") {
            List<Map<String, Object>> list = dao.phoneChange(query.get("phone").toString());*/
    ...
    public Map<String, Object> addUser(Users users) {
        ...
        int flag = 0;
        try {
            //初始登录名为手机号，初始密码为123456  盐值和加密后的密码 添加user_role
    ...
    /*public String test1(ModelMap mm) throws Exception {
        InputStream in = new FileInputStream("d://aa.xls");
        Map<String, String> fieldd = new HashMap<String, String>();
        fieldd.put("学号", "uid");
        fieldd.put("姓名", "uname");
        fieldd.put("性别", "sex");
        fieldd.put("民族", "nation");
        List<Student> resultList = new ArrayList<Student>();
        resultList = ExeclUtil.ExecltoList(in, Student.class, fieldd);
        mm.put("lists", resultList);
        return "index";
    }*/
    ...
    /*HashMap<String, Object> m = new HashMap<>();
        int ret = 0;
    for(Map<String, Object> map : inExcelUsers){
            //判断手机号是否已经存在
            Map<String, Object> phone = new HashMap<>();
            phone.put("phone",map.get("phone").toString());
            Map<String, Object> stringObjectMap = phoneChange(phone);
            if(stringObjectMap.get("phoneChangeInfo").equals("合格")){
                Users users = new Users();
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

                    //初始登录名为手机号，初始密码为123456  盐值和加密后的密码
                    users.setLoginName(map.get("phone").toString());
                    users.setPassword("123456");
                    LxlShiroUtil.encryptPassword(users);
                    ...
                    users.setAge(Integer.parseInt(map.get("age").toString()));
                    users.setPhone(map.get("phone").toString());
                    if(map.get("status").toString().equals("在职")){
                        users.setStatus(0);
                    }
                    if(map.get("status").toString().equals("离职")){
                        users.setStatus(1);
                    }
            try {
                    users.setCreateTime(sdf.parse(map.get("createTime").toString()));
                } catch (Exception e) {
                    e.printStackTrace();
                }
